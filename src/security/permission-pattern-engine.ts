 ...
 *
 * Relocated from src/loop-engine/harness/ to src/security/ so it can be shared by
 * the AuthorizationPipeline and other security infrastructure (task 2.2).
 *
 ...
 *
 * Requirements: 10.4, 10.5, 19.1, 19.2, 19.3, 19.4, 19.5, 19.6, 19.7
 */
...
export type HierarchyLevel = 'managed' | 'project' | 'local' | 'per-agent' | 'user';
...
export class PermissionPatternEngine {
  ...
  private perAgentPatterns: Map<string, PermissionConfig> = new Map();
   ...
   *   2. Allow match (in hierarchy order: managed → project → local → per-agent → user) → 'allow'
   *   3. If loop is active (REQ-19.7) and not denied → 'allow'
   *   4. No match → 'no-match' (falls through to Action_Security_Analyzer)
   */
  evaluate(toolName: string, args: string, agentId?: string): PatternDecision {
    ...
    // Check deny patterns from all hierarchy levels (including per-agent)
    const agentConfig = agentId ? this.perAgentPatterns.get(agentId) : undefined;
    ...
    // Managed first (highest priority), then project, local, per-agent, user (REQ-19.4, Req 10.4)
    for (const config of allConfigs) {
  ...
  /**
   * Set per-agent patterns for a specific agent.
   * Per-agent patterns sit between local and user tiers in priority (Req 10.4, 10.11).
   */
  setPerAgentPatterns(agentId: string, config: PermissionConfig): void {
    this.perAgentPatterns.set(agentId, { ...config });
  }

  /**
   * Set per-agent patterns for multiple agents at once (bulk load).
   * Replaces all existing per-agent patterns.
   */
  setAllPerAgentPatterns(patterns: Map<string, PermissionConfig>): void {
    this.perAgentPatterns = new Map(patterns);
  }

  /**
   * Get per-agent patterns for a specific agent (for inspection/debugging).
   */
  getPerAgentPatterns(agentId: string): PermissionConfig | undefined {
    return this.perAgentPatterns.get(agentId);
  }

  /**
   * Get all per-agent pattern entries (for inspection/debugging).
   */
  getAllPerAgentPatterns(): Map<string, PermissionConfig> {
    return new Map(this.perAgentPatterns);
  }
  ...
  getAllDenyPatterns(): string[] {
    const perAgentDeny: string[] = [];
    for (const config of this.perAgentPatterns.values()) {
      perAgentDeny.push(...config.deny);
    }
    return [
      ...this.neverTouchDeny,
  ...
  getAllAllowPatterns(): string[] {
    const perAgentAllow: string[] = [];
    for (const config of this.perAgentPatterns.values()) {
      perAgentAllow.push(...config.allow);
    }
    return [
