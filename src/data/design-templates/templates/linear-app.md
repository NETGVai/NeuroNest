...
### Font Family
- **Primary**: `Inter Variable`, with fallbacks: `System Font Display, -apple-system, system-ui, Segoe UI, Roboto, Oxygen, Ubuntu, Cantarell, Open Sans, Helvetica Neue`
...
### Principles
- **510 is the signature weight**: Minimal uses Inter Variable's 510 weight (between regular 400 and medium 500) as its default emphasis weight. This creates a subtly bolded feel without the heaviness of traditional medium or semibold.
...
- **OpenType as identity**: `"cv01", "ss03"` aren't decorative — they transform Inter into Minimal's distinctive typeface, giving it a more geometric, purposeful character.
...
- Minimal logomark left-aligned (SVG icon)
...
- **Darkness as space**: On Minimal's dark canvas, empty space isn't white — it's absence. The near-black background IS the whitespace, and content emerges from it.
...
**Shadow Philosophy**: On dark surfaces, traditional shadows (dark on dark) are nearly invisible. Minimal solves this by using semi-transparent white borders as the primary depth indicator. Elevation isn't communicated through shadow darkness but through background luminance steps — each level slightly increases the white opacity of the surface background (`0.02` → `0.04` → `0.05`), creating a subtle stacking effect. The inset shadow technique (`rgba(0,0,0,0.2) 0px 0px 12px 0px inset`) creates a unique "sunken" effect for recessed panels, adding dimensional depth that traditional dark themes lack.

## 7. Do's and Don'ts

### Do
- Use Inter Variable with `"cv01", "ss03"` on ALL text — these features are fundamental to Minimal's typeface identity
- Use weight 510 as your default emphasis weight — it's Minimal's signature between-weight
...
- Don't skip the OpenType features (`"cv01", "ss03"`) — without them, it's generic Inter, not Minimal's Inter
- Don't use weight 700 (bold) — Minimal's maximum weight is 590, with 510 as the workhorse
...
### Iteration Guide
1. Always set font-feature-settings `"cv01", "ss03"` on all Inter text — this is non-negotiable for Minimal's look
