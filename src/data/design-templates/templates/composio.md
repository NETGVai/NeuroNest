...
### Primary
- **Integration Cobalt** (`#0007cd`): The core brand color — a deep, saturated blue used sparingly for high-priority interactive elements and brand moments. It anchors the identity with quiet intensity.
...
- **Blue-to-Black Fade**: linear gradients from Integration Cobalt (`#0007cd`) fading into Void Black (`#0f0f0f`), used in hero backgrounds and section transitions.
...
### Font Family
- **Primary**: `abcDiatype`, with fallbacks: `abcDiatype Fallback, ui-sans-serif, system-ui, Color Emoji, Segoe UI Emoji, Segoe UI Symbol, Noto Color Emoji`
...
- Logo (white SVG): Integration wordmark on the left
...
- **Contrast-driven separation**: Rather than relying solely on whitespace, Integration uses border opacity differences and subtle background shifts to delineate content zones.
...
**Shadow Philosophy**: Integration uses shadows sparingly and with deliberate contrast. The hard-offset brutalist shadow is the signature — it breaks the sleek darkness with a raw, almost retro-computing feel. The soft diffuse shadow is reserved for truly floating elements. Most depth is communicated through border opacity gradations rather than shadows.
...
- **Blue-Black Gradient Washes**: linear gradients from Integration Cobalt to Void Black used as section backgrounds, adding subtle color temperature shifts.
...
- Don't use Integration Cobalt (`#0007cd`) as a text color — it's too dark on dark and too saturated on light
...
- Brand Accent: "Integration Cobalt (#0007cd)"
...
- "Design a navigation bar on Void Black with the Integration wordmark in white on the left, 4-5 nav links in white abcDiatype at 16px, and a white-fill CTA button on the right. Add a Border Mist 06 bottom border."
