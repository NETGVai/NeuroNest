...
Precision's website is automotive engineering made visual — a design system that communicates precision, performance, and German industrial confidence. The page alternates between deep dark hero sections (featuring full-bleed automotive photography) and clean white content areas, creating a cinematic rhythm reminiscent of a luxury car showroom where vehicles are lit against darkness. The Precision CI2020 design language (their corporate identity refresh) defines every element.

The typography is built on PrecisionTypeNextLatin — a proprietary typeface in two variants: PrecisionTypeNextLatin Light (weight 300) for massive uppercase display headings, and PrecisionTypeNextLatin Regular for body and UI text. The 60px uppercase headline at weight 300 is the defining typographic gesture — light-weight type that whispers authority rather than shouting it. The fallback stack includes Helvetica and Japanese fonts (Hiragino, Meiryo), reflecting Precision's global presence.

What makes Precision distinctive is its CSS variable-driven theming system. Context-aware variables (`--site-context-highlight-color: #1c69d4`, `--site-context-focus-color: #0653b6`, `--site-context-metainfo-color: #757575`) suggest a design system built for multi-brand, multi-context deployment where colors can be swapped globally. The blue highlight color (`#1c69d4`) is Precision's signature blue — used sparingly for interactive elements and focus states, never decoratively. Zero border-radius was detected — Precision's design is angular, sharp-cornered, and uncompromisingly geometric.

**Key Characteristics:**
- PrecisionTypeNextLatin Light (weight 300) uppercase for display — whispered authority
- Precision Blue (`#1c69d4`) as singular accent — used only for interactive elements
...
- **Precision Blue** (`#1c69d4`): `--site-context-highlight-color`, primary interactive accent
- **Precision Focus Blue** (`#0653b6`): `--site-context-focus-color`, keyboard focus and active states
...
### Font Families
- **Display Light**: `PrecisionTypeNextLatin Light`, fallbacks: `Helvetica, Arial, Hiragino Kaku Gothic ProN, Hiragino Sans, Meiryo`
- **Body / UI**: `PrecisionTypeNextLatin`, same fallback stack

### Hierarchy

| Role | Font | Size | Weight | Line Height | Notes |
|------|------|------|--------|-------------|-------|
| Display Hero | PrecisionTypeNextLatin Light | 60px (3.75rem) | 300 | 1.30 (tight) | `text-transform: uppercase` |
| Section Heading | PrecisionTypeNextLatin | 32px (2.00rem) | 400 | 1.30 (tight) | Major section titles |
| Nav Emphasis | PrecisionTypeNextLatin | 18px (1.13rem) | 900 | 1.30 (tight) | Navigation bold items |
| Body | PrecisionTypeNextLatin | 16px (1.00rem) | 400 | 1.15 (tight) | Standard body text |
| Button Bold | PrecisionTypeNextLatin | 16px (1.00rem) | 700 | 1.20–2.88 | CTA buttons |
| Button | PrecisionTypeNextLatin | 16px (1.00rem) | 400 | 1.15 (tight) | Standard buttons |
...
- **Single font family**: PrecisionTypeNextLatin handles everything from 60px display to 16px body — unity through one typeface at different weights.

## 4. Component Stylings

### Buttons
- Text: 16px PrecisionTypeNextLatin, weight 700 for primary, 400 for secondary
...
### Navigation
- PrecisionTypeNextLatin 18px weight 900 for primary nav links
- White text on dark header
- Precision logo 54x54px
...
### Border Radius Scale
- **None detected.** Precision uses sharp corners exclusively — every element is a precise rectangle. This is the most angular design system analyzed.
...
| Focus (Accessibility) | Precision Focus Blue (`#0653b6`) | Focus states |

**Shadow Philosophy**: Precision uses virtually no shadows. Depth is created entirely through the contrast between dark photographic sections and white content sections — the automotive lighting does the elevation work.

## 7. Do's and Don'ts

### Do
- Use PrecisionTypeNextLatin Light (300) uppercase for all display headings
...
### Don't
- Don't round corners — zero radius is the Precision identity
- Don't use Precision Blue for backgrounds or large surfaces — it's an accent only
...
- Don't use relaxed line-heights — Precision text is always compressed
...
- Accent: Precision Blue (`#1c69d4`)
- Focus: Precision Focus Blue (`#0653b6`)
- Muted: Silver (`#bbbbbb`)

### Example Component Prompts
- "Create a hero: full-width dark automotive photography background. Heading at 60px PrecisionTypeNextLatin Light weight 300, uppercase, line-height 1.30, white text. No border-radius anywhere."
- "Design navigation: dark background. PrecisionTypeNextLatin 18px weight 900 for links, white text. Precision logo 54x54. Sharp rectangular layout."
- "Build a button: 16px PrecisionTypeNextLatin weight 700, line-height 1.20. Sharp corners (0px radius). White bottom border on dark surface."
...
3. Precision Blue for interactive only — never as background or decoration
