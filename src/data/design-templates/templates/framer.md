...
- Motion Blue (`#0099ff`) as the sole accent color — cold, electric, precise
...
### Primary
- **Pure Black** (`#000000`): Primary background, the void canvas that defines Motion's dark-first identity
- **Pure White** (`#ffffff`): Primary text color on dark surfaces, button text on accent backgrounds
- **Motion Blue** (`#0099ff`): Primary accent color — links, borders, ring shadows, interactive highlights
...
### Semantic & Accent
- **Motion Blue** (`#0099ff`): Links, interactive borders, focus rings
...
### Gradient System
- No prominent gradient usage — Motion relies on pure flat black surfaces with occasional blue-tinted glows for depth
- Subtle radial glow effects behind product screenshots using Motion Blue at very low opacity

## 3. Typography Rules

### Font Family
- **Display**: `GT Walsheim Motion Medium` / `GT Walsheim Medium` — custom geometric sans-serif, weight 500. Fallbacks: `GT Walsheim Motion Medium Placeholder`, system sans-serif
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | GT Walsheim Motion Medium | 110px | 500 | 0.85 | -5.5px | Extreme negative tracking, compressed impact |
...
| Body Readable | Inter Motion Regular | 14px | 400 | 1.60 | normal | Long-form body text |
...
- **Hover**: Subtle glow increase on Motion Blue ring shadow, or brightness shift on frosted surfaces

### Inputs & Forms
- Minimal form presence on the marketing site
- Input fields follow dark theme: dark background, subtle border, white text
- Focus state: Motion Blue (`#0099ff`) ring border, `1px solid #0099ff`
...
| Level 1 (Ring) | `rgba(0, 153, 255, 0.15) 0px 0px 0px 1px` | Card borders, interactive element outlines — Motion Blue glow ring |
...
### Shadow Philosophy
Motion's elevation system is inverted from traditional light-theme designs. Instead of darker shadows on light backgrounds, Motion uses:
...
### Decorative Depth
- **Blue glow auras**: Subtle Motion Blue (`#0099ff`) radial gradients behind key interactive areas
...
- Use Motion Blue (`#0099ff`) exclusively for interactive accents — links, borders, focus states
...
- Introduce additional accent colors beyond Motion Blue — this is a one-accent-color system
...
- "Design a feature card on black background with a 1px Motion Blue ring shadow border (rgba(0,153,255,0.15)), 12px border-radius, white heading in Inter at 22px weight 700, and muted silver (a6a6a6) body text"
...
- "Design a pricing card using pure black surface, Motion Blue (#0099ff) accent for the selected plan border, white text hierarchy (24px Inter bold heading, 14px regular body), and a solid white pill CTA button"
...
3. Check that Motion Blue appears ONLY on interactive elements — never as decorative background or text color for non-links
4. Ensure all buttons are pill-shaped — any squared corner immediately breaks the Motion aesthetic
