...
Music's web interface is a dark, immersive music player that wraps listeners in a near-black cocoon (`#121212`, `#181818`, `#1f1f1f`) where album art and content become the primary source of color. The design philosophy is "content-first darkness" — the UI recedes into shadow so that music, podcasts, and playlists can glow. Every surface is a shade of charcoal, creating a theater-like environment where the only true color comes from the iconic Music Green (`#1ed760`) and the album artwork itself.

The typography uses MusicMixUI and MusicMixUITitle — proprietary fonts from the CircularSp family (Circular by Lineto, customized for Music) with an extensive fallback stack that includes Arabic, Hebrew, Cyrillic, Greek, Devanagari, and CJK fonts, reflecting Music's global reach. The type system is compact and functional: 700 (bold) for emphasis and navigation, 600 (semibold) for secondary emphasis, and 400 (regular) for body. Buttons use uppercase with positive letter-spacing (1.4px–2px) for a systematic, label-like quality.

What distinguishes Music is its pill-and-circle geometry. Primary buttons use 500px–9999px radius (full pill), circular play buttons use 50% radius, and search inputs are 500px pills. Combined with heavy shadows (`rgba(0,0,0,0.5) 0px 8px 24px`) on elevated elements and a unique inset border-shadow combo (`rgb(18,18,18) 0px 1px 0px, rgb(124,124,124) 0px 0px 0px 1px inset`), the result is an interface that feels like a premium audio device — tactile, rounded, and built for touch.

**Key Characteristics:**
- Near-black immersive dark theme (`#121212`–`#1f1f1f`) — UI disappears behind content
- Music Green (`#1ed760`) as singular brand accent — never decorative, always functional
- MusicMixUI/CircularSp font family with global script support
...
### Primary Brand
- **Music Green** (`#1ed760`): Primary brand accent — play buttons, active states, CTAs
...
- **Music Green Border** (`#1db954`): Green accent border variant
...
### Font Families
- **Title**: `MusicMixUITitle`, fallbacks: `CircularSp-Arab, CircularSp-Hebr, CircularSp-Cyrl, CircularSp-Grek, CircularSp-Deva, Helvetica Neue, helvetica, arial, Hiragino Sans, Hiragino Kaku Gothic ProN, Meiryo, MS Gothic`
- **UI / Body**: `MusicMixUI`, same fallback stack

### Hierarchy

| Role | Font | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|------|--------|-------------|----------------|-------|
| Section Title | MusicMixUITitle | 24px (1.50rem) | 700 | normal | normal | Bold title weight |
| Feature Heading | MusicMixUI | 18px (1.13rem) | 600 | 1.30 (tight) | normal | Semibold section heads |
| Body Bold | MusicMixUI | 16px (1.00rem) | 700 | normal | normal | Emphasized text |
| Body | MusicMixUI | 16px (1.00rem) | 400 | normal | normal | Standard body |
| Button Uppercase | MusicMixUI | 14px (0.88rem) | 600–700 | 1.00 (tight) | 1.4px–2px | `text-transform: uppercase` |
| Button | MusicMixUI | 14px (0.88rem) | 700 | normal | 0.14px | Standard button |
| Nav Link Bold | MusicMixUI | 14px (0.88rem) | 700 | normal | normal | Navigation |
| Nav Link | MusicMixUI | 14px (0.88rem) | 400 | normal | normal | Inactive nav |
| Caption Bold | MusicMixUI | 14px (0.88rem) | 700 | 1.50–1.54 | normal | Bold metadata |
| Caption | MusicMixUI | 14px (0.88rem) | 400 | normal | normal | Metadata |
| Small Bold | MusicMixUI | 12px (0.75rem) | 700 | 1.50 | normal | Tags, counts |
| Small | MusicMixUI | 12px (0.75rem) | 400 | normal | normal | Fine print |
| Badge | MusicMixUI | 10.5px (0.66rem) | 600 | 1.33 | normal | `text-transform: capitalize` |
| Micro | MusicMixUI | 10px (0.63rem) | 400 | normal | normal | Smallest text |
...
- **Compact sizing**: The range is 10px–24px — narrower than most systems. Music's type is compact and functional, designed for scanning playlists, not reading articles.
- **Global script support**: The extensive fallback stack (Arabic, Hebrew, Cyrillic, Greek, Devanagari, CJK) reflects Music's 180+ market reach.
...
### Navigation
- Dark sidebar with MusicMixUI 14px weight 700 for active, 400 for inactive
- `#b3b3b3` muted color for inactive items, `#ffffff` for active
- Circular icon buttons (50% radius)
- Music logo top-left in green
...
- **Dark compression**: Music packs content densely — playlist grids, track lists, and navigation are all tightly spaced. The dark background provides visual rest between elements without needing large gaps.
...
**Shadow Philosophy**: Music uses notably heavy shadows for a dark-themed app. The 0.5 opacity shadow at 24px blur creates a dramatic "floating in darkness" effect for dialogs and menus, while the 0.3 opacity at 8px blur provides a more subtle card lift. The unique inset border-shadow combination on inputs creates a recessed, tactile quality.

## 7. Do's and Don'ts

### Do
- Use near-black backgrounds (`#121212`–`#1f1f1f`) — depth through shade variation
- Apply Music Green (`#1ed760`) only for play controls, active states, and primary CTAs
...
### Don't
- Don't use Music Green decoratively or on backgrounds — it's functional only
...
- Don't use relaxed line-heights — Music's typography is compact and dense
...
- Accent: Music Green (`#1ed760`)
- Border: `#4d4d4d`
- Error: Negative Red (`#f3727f`)

### Example Component Prompts
- "Create a dark card: #181818 background, 8px radius. Title at 16px MusicMixUI weight 700, white text. Subtitle at 14px weight 400, #b3b3b3. Shadow rgba(0,0,0,0.3) 0px 8px 8px on hover."
- "Design a pill button: #1f1f1f background, white text, 9999px radius, 8px 16px padding. 14px MusicMixUI weight 700, uppercase, letter-spacing 1.4px."
- "Build a circular play button: Music Green (#1ed760) background, #000000 icon, 50% radius, 12px padding."
- "Create search input: #1f1f1f background, white text, 500px radius, 12px 48px padding. Inset border: rgb(124,124,124) 0px 0px 0px 1px inset."
- "Design navigation sidebar: #121212 background. Active items: 14px weight 700, white. Inactive: 14px weight 400, #b3b3b3."

### Iteration Guide
1. Start with #121212 — everything lives in near-black darkness
2. Music Green for functional highlights only (play, active, CTA)
