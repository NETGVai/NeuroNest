...
Urban's design language is a masterclass in confident minimalism -- a black-and-white universe where every pixel serves a purpose and nothing decorates without earning its place. The entire experience is built on a stark duality: jet black (`#000000`) and pure white (`#ffffff`), with virtually no mid-tone grays diluting the message. This isn't the sterile minimalism of a startup that hasn't finished designing -- it's the deliberate restraint of a brand so established it can afford to whisper.

The signature typeface, UrbanMove, is a proprietary geometric sans-serif with a distinctly square, engineered quality. Headlines in UrbanMove Bold at 52px carry the weight of a billboard -- authoritative, direct, unapologetic. The companion face UrbanMoveText handles body copy and buttons with a slightly softer, more readable character at medium weight (500). Together, they create a typographic system that feels like a transit map: clear, efficient, built for scanning at speed.

What makes Urban's design truly distinctive is its use of full-bleed photography and illustration paired with pill-shaped interactive elements (999px border-radius). Navigation chips, CTA buttons, and category selectors all share this capsule shape, creating a tactile, thumb-friendly interface language that's unmistakably Urban. The illustrations -- warm, slightly stylized scenes of drivers, riders, and cityscapes -- inject humanity into what could otherwise be a cold, monochrome system. The site alternates between white content sections and a full-black footer, with card-based layouts using the gentlest possible shadows (rgba(0,0,0,0.12-0.16)) to create subtle lift without breaking the flat aesthetic.

**Key Characteristics:**
- Pure black-and-white foundation with virtually no mid-tone grays in the UI chrome
- UrbanMove (headlines) + UrbanMoveText (body/UI) -- proprietary geometric sans-serif family
...
### Primary
- **Urban Black** (`#000000`): The defining brand color -- used for primary buttons, headlines, navigation text, and the footer. Not "near-black" or "off-black," but true, uncompromising black.
...
### Gradient System
- Urban's design is **entirely gradient-free**. The black/white duality and flat color blocks create all visual hierarchy. No gradients appear anywhere in the system -- every surface is a solid color, every transition is a hard edge or a shadow.

## 3. Typography Rules

### Font Family
- **Headline / Display**: `UrbanMove`, with fallbacks: `UrbanMoveText, system-ui, Helvetica Neue, Helvetica, Arial, sans-serif`
- **Body / UI**: `UrbanMoveText`, with fallbacks: `system-ui, Helvetica Neue, Helvetica, Arial, sans-serif`

*Note: UrbanMove and UrbanMoveText are proprietary typefaces. For external implementations, use `system-ui` or Inter as the closest available substitute. The geometric, square-proportioned character of UrbanMove can be approximated with Inter or DM Sans.*

### Hierarchy

| Role | Font | Size | Weight | Line Height | Notes |
|------|------|------|--------|-------------|-------|
| Display / Hero | UrbanMove | 52px (3.25rem) | 700 | 1.23 (tight) | Maximum impact, billboard presence |
| Section Heading | UrbanMove | 36px (2.25rem) | 700 | 1.22 (tight) | Major section anchors |
| Card Title | UrbanMove | 32px (2rem) | 700 | 1.25 (tight) | Card and feature headings |
| Sub-heading | UrbanMove | 24px (1.5rem) | 700 | 1.33 | Secondary section headers |
| Small Heading | UrbanMove | 20px (1.25rem) | 700 | 1.40 | Compact headings, list titles |
| Nav / UI Large | UrbanMoveText | 18px (1.13rem) | 500 | 1.33 | Navigation links, prominent UI text |
| Body / Button | UrbanMoveText | 16px (1rem) | 400-500 | 1.25-1.50 | Standard body text, button labels |
| Caption | UrbanMoveText | 14px (0.88rem) | 400-500 | 1.14-1.43 | Metadata, descriptions, small links |
| Micro | UrbanMoveText | 12px (0.75rem) | 400 | 1.67 (relaxed) | Fine print, legal text |

### Principles
- **Bold headlines, medium body**: UrbanMove headings are exclusively weight 700 (bold) -- every headline hits with billboard force. UrbanMoveText body and UI text uses 400-500, creating a clear visual hierarchy through weight contrast.
...
- **Two fonts, strict roles**: UrbanMove is exclusively for headings. UrbanMoveText is exclusively for body, buttons, links, and UI. The boundary is never crossed.
...
- Logo: Urban wordmark/icon at 24x24px in black
- Links: UrbanMoveText at 14-18px, weight 500, in Urban Black
- Pill-shaped nav chips with Chip Gray (`#efefef`) background for category navigation ("Ride", "Drive", "Business", "Urban Eats")
...
- Horizontal row of pill-shaped buttons for top-level navigation ("Ride", "Drive", "Business", "Urban Eats", "About")
...
**Plan-Ahead Cards**
- Cards promoting features like "Urban Reserve" and trip planning
...
- **Efficient, not airy**: Urban's whitespace is functional -- enough to separate, never enough to feel empty. This is transit-system spacing: compact, clear, purpose-driven.
...
- Use UrbanMoveText at weight 500 for navigation and prominent UI text
- Pair black primary buttons with white secondary buttons for dual-action layouts

### Don't
- Don't introduce color into the UI chrome -- Urban's interface is strictly black, white, and gray
...
- Don't use serif fonts anywhere -- Urban's typography is exclusively geometric sans-serif
- Don't create airy, spacious layouts with excessive whitespace -- Urban's density is intentional
- Don't use gradients or color overlays -- every surface is a flat, solid color
- Don't mix UrbanMove into body text or UrbanMoveText into headlines -- the hierarchy is strict
...
- Primary Button: "Urban Black (#000000)"
- Page Background: "Pure White (#ffffff)"
- Button Text (on black): "Pure White (#ffffff)"
- Button Text (on white): "Urban Black (#000000)"
...
- Footer Background: "Urban Black (#000000)"

### Example Component Prompts
- "Create a hero section on Pure White (#ffffff) with a headline at 52px UrbanMove Bold (700), line-height 1.23. Use Urban Black (#000000) text. Add a subtitle in Body Gray (#4b4b4b) at 16px UrbanMoveText weight 400 with 1.50 line-height. Place an Urban Black (#000000) pill CTA button with Pure White text, 999px radius, padding 10px 12px."
- "Design a category navigation bar with horizontal pill buttons. Each pill: Chip Gray (#efefef) background, Urban Black (#000000) text, 14px 16px padding, 999px border-radius. Active pill inverts to Urban Black background with Pure White text. Use UrbanMoveText at 14px weight 500."
- "Build a feature card on Pure White (#ffffff) with 8px border-radius and shadow rgba(0,0,0,0.12) 0px 4px 16px. Title in UrbanMove at 24px weight 700, description in Body Gray (#4b4b4b) at 16px UrbanMoveText. Add a black pill CTA button at the bottom."
- "Create a dark footer on Urban Black (#000000) with Pure White (#ffffff) heading text in UrbanMove at 20px weight 700. Footer links in Muted Gray (#afafaf) at 14px UrbanMoveText. Links hover to Pure White. Multi-column grid layout."
...
2. Reference the strict black/white palette -- "use Urban Black (#000000)" not "make it dark"
3. Always specify 999px radius for buttons and pills -- this is non-negotiable for the Urban identity
4. Describe the font family explicitly -- "UrbanMove Bold for the heading, UrbanMoveText Medium for the label"
5. For shadows, use "whisper shadow (rgba(0,0,0,0.12) 0px 4px 16px)" -- never heavy drop shadows
6. Keep layouts compact and information-dense -- Urban is efficient, not airy
