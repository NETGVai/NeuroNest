...
- **Void Mint** (`#2fd6a1`): The button-text variant of the brand green — slightly warmer and more readable than pure Signal Green, used specifically for CTA text on dark surfaces.
- **Tailwind Emerald** (`#10b981`): The ecosystem-standard green used at low opacity (30%) for subtle background tints and link defaults. Bridges Void's custom palette with Tailwind's utility classes.
...
### Gradient System
- **Green Signal Glow**: `drop-shadow(0 0 2px #00d992)` animating to `drop-shadow(0 0 8px #00d992)` — creates a pulsing "electric charge" effect on the Void bolt logo and interactive elements. The glow expands and contracts like a heartbeat.
...
### Font Family
- **Primary (Headings)**: `system-ui`, with fallbacks: `-apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans, Helvetica, Arial, Color Emoji, Segoe UI Emoji, Segoe UI Symbol`
...
### Principles
- **System-native authority**: Display headings use system-ui rather than a custom web font — this means the largest text renders instantly (no FOIT/FOUT) and inherits the operating system's native personality. On macOS it's System Font, on Windows it's Segoe UI. The design accepts this variability as a feature, not a bug.
...
- Text: Void Mint (`#2fd6a1`)
- Padding: comfortable (12px 16px)
- Border: none visible (outline-based focus indicator)
- Outline: Void Mint (`rgb(47, 214, 161)`)
...
- Inferred style: Carbon Surface background, Warm Charcoal border, Void Mint focus ring, Snow White text

### Navigation
- Sticky top nav bar on Abyss Black canvas
- Logo: Void bolt icon with animated green glow (`drop-shadow` cycling 2px–8px)
...
- Connection lines use Void green variants
...
- **Border-defined separation**: Rather than relying solely on whitespace, Void uses the Warm Charcoal border system (`#3d3a39`) to delineate content zones. The border IS the whitespace signal.
...
**Shadow Philosophy**: Void communicates depth primarily through **border weight and color**, not shadows. The standard `1px solid #3d3a39` border IS the elevation — adding a `3px` border weight or switching to green (`#00d992`) communicates importance more than adding shadow does. When shadows do appear, they're either warm and diffused (Level 4) or cinematic and dramatic (Level 5) — never medium or generic.

### Decorative Depth
- **Green Signal Glow**: The Void bolt logo pulses with a `drop-shadow` animation cycling between 2px and 8px blur radius in Emerald Signal Green. This is the most distinctive decorative element — it makes the logo feel "powered on."
...
- Use Void Mint (`#2fd6a1`) for button text on dark surfaces — it's more readable than pure Signal Green
...
- "Design a navigation bar on Abyss Black (#050507) with the Void logo (bolt icon with animated green glow) on the left, nav links in Inter at 14px weight 500 in Snow White, and a green CTA button (Carbon Surface bg, Void Mint text) on the right. Add a 1px solid Warm Charcoal bottom border."
