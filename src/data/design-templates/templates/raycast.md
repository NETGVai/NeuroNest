...
- Launcher Red (`#FF6363`) as a punctuation color — hero stripes, not pervasive
...
- **Launcher Red** (`#FF6363` / `hsl(0, 100%, 69%)`): Brand accent — hero stripes, danger states, critical highlights

### Secondary & Accent
- **Launcher Blue** (`hsl(202, 100%, 67%)` / ~`#55b3ff`): Interactive accent — links, focus states, selected items
- **Launcher Green** (`hsl(151, 59%, 59%)` / ~`#5fc992`): Success states, positive indicators
- **Launcher Yellow** (`hsl(43, 100%, 60%)` / ~`#ffbc33`): Warning accents, highlights
...
- **System**: `System Font Text` — system font for select macOS-native UI elements. Fallbacks: `System Font Icons`, `Inter`, `Inter Fallback`
- **Monospace**: `GeistMono` — the platform's monospace font for code elements. Fallbacks: `ui-monospace`, `SFMono-Regular`, `Roboto Mono`, `Menlo`, `Monaco`
...
### Principles
- **Positive tracking on dark**: Unlike most dark UIs that use tight or neutral letter-spacing, Launcher applies +0.2px to +0.4px — creating an airy, readable feel that compensates for the dark background
...
- **Display restraint**: Hero text at 64px/600 is confident but not oversized — Launcher avoids typographic spectacle in favor of functional elegance
...
- **Transition**: All buttons use opacity transition for hover rather than background-color change — a signature Launcher interaction pattern
...
- **Hero illustration**: Diagonal stripe pattern in Launcher Red — abstract, geometric, brand-defining
- **App UI embeds**: Showing actual Launcher command palette and extensions — product as content
...
### Shadow Philosophy
Launcher's shadow system is the most macOS-native on the web. Multi-layer shadows combine:
...
### Do
- Use `#07080a` (not pure black) as the background — the blue-cold tint is essential to the Launcher feel
...
- Keep Launcher Red (`#FF6363`) as punctuation, not pervasive — reserve it for hero moments and error states
...
### Don't
- Use pure black (`#000000`) as the background — the blue tint differentiates Launcher from generic dark themes
- Apply negative letter-spacing on body text — Launcher deliberately uses positive spacing for readability
- Use Launcher Blue as the primary accent for everything — blue is for interactive/info, red is the brand color
...
- Brand Accent: Launcher Red (`#FF6363`)
- Interactive Blue: Launcher Blue (`hsl(202, 100%, 67%)` / ~`#55b3ff`)
...
- "Design an alert card with #101111 surface, Launcher Red (#FF6363) left border accent, translucent red glow (hsla(0,100%,69%,0.15)), white heading, and #cecece description text"

### Iteration Guide
When refining existing screens generated with this design system:
1. Check the background is `#07080a` not pure black — the blue tint is critical
2. Verify letter-spacing is positive (+0.2px) on body text — negative spacing breaks the Launcher aesthetic
