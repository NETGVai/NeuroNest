...
- **No heavy shadows**: Docs relies on borders, not shadows, for depth.
...
- **Documentation-grade breathing room**: Every element has generous surrounding whitespace. Docs sells documentation, so the marketing page itself demonstrates reading comfort.
...
- **Content density is low**: Unlike developer tools that pack the page, Docs uses 1–2 key messages per section with supporting imagery.
...
### Iteration Guide
1. Always use full-pill radius (9999px) for buttons and inputs — this is Docs's signature shape
