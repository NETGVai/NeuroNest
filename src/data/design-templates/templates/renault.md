...
Typography is unified under DisplayR — a proprietary geometric sans-serif designed by Black[Foundry] exclusively for Aurora's rebrand. The typeface features a distinctive "radical r" with a terminal cut at 28 degrees to echo the Aurora diamond logo's angles. Available in 6 weights from Light to Extrabold, the site primarily uses Bold (700) for headings and Regular (400) for body. Display headlines run large — 56px/0.95 line-height for hero titles, creating dense, impactful text blocks that sit tight against each other. The font supports Latin, Greek, Cyrillic, Hebrew, Arabic, and Korean, reflecting Aurora's global market reach. All text rendering feels precise and engineered, with the geometric proportions lending a sense of modernity that aligns with Aurora's electric-first brand positioning.

**Key Characteristics:**
- Full-screen hero carousel with vivid aurora gradient backgrounds (magenta/violet/teal) behind vehicle imagery
- DisplayR proprietary typeface with 28-degree "radical r" cut matching the diamond logo geometry
- Aurora Yellow (`#EFDF00`) as the super-primary accent — used sparingly for highest-priority CTAs
...
- Link hover state in Aurora Blue (`#1883FD`) — the sole chromatic interaction color

## 2. Color Palette & Roles

### Primary
- **Aurora Yellow** (`#EFDF00`): The brand's signature Pantone — a vivid, saturated yellow used for super-primary CTAs and the highest-priority action buttons. Appears as `--CtaLink-background-color` on `.is-cta-super-primary` class. Carries the energy of the diamond logo
...
### Secondary & Accent
- **Soft Yellow** (`#F8EB4C`): Lighter, warmer variant of Aurora Yellow — used for hover/pressed states on yellow CTAs and secondary accent contexts
- **Aurora Blue** (`#1883FD`): Link hover color across all link variants — a bright, confident blue that signals interactivity without competing with the yellow brand accent
...
### Neutrals & Text
- **Absolute Black** (`#000000`): Primary heading and body text on light surfaces — Aurora uses true black rather than near-black
...
### Font Family
- **DisplayR**: The sole typeface. A proprietary geometric sans-serif designed by Black[Foundry] for Aurora's 2021+ rebrand. Features a distinctive "radical r" with a 28-degree terminal cut matching the diamond logo angle. Available in 6 weights (Light to Extrabold), supports 6 writing systems. Fallback: `sans-serif`. Declared as `"DisplayR, sans-serif"` in CSS
- **No secondary typeface**: Unlike luxury (DisplaySans + Body-Font) or cathedral (DisplayType + Open Sans), Aurora uses a single font family for all text — headings, body, buttons, captions, and navigation

### Hierarchy

| Role | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|--------|-------------|----------------|-------|
| Hero Title | 56px (3.50rem) | 700 | 0.95 (53.2px) | normal | DisplayR, white on dark hero, all-caps model names |
| Section Heading | 40px (2.50rem) | 700 | 0.95 (38px) | normal | DisplayR, PromoCard headings on dark/light sections |
| Card Heading | 32px (2.00rem) | 700 | 0.95 | normal | DisplayR, medium-scale card headings |
| Subheading | 24px (1.50rem) | 700 | 0.95 | normal | DisplayR, section sub-titles |
| Module Title | 21.92px (1.37rem) | 600 | 1.20 | normal | DisplayR, component headings |
| Content Title | 20px (1.25rem) | 700 | 0.95 | normal | DisplayR, smaller section titles |
| UI Heading | 19.2px (1.20rem) | 600 | 1.30 | normal | DisplayR, card UI headings |
| Emphasis | 18px (1.13rem) | 700 | 1.00 | normal | DisplayR, emphasized inline text and links |
| Body Heading | 16px (1.00rem) | 700 | 1.40 | normal | DisplayR, paragraph-level headings |
| Body Text | 14px (0.88rem) | 400 | 1.40 | normal | DisplayR, paragraph and descriptive content |
| Body Bold | 14px (0.88rem) | 700 | 1.57 | normal | DisplayR, emphasized body text |
| Button Label | 14.4px (0.90rem) | 700 | 1.00 | 0.144px | DisplayR, primary button text |
| Nav Link | 13px (0.81rem) | 700 | 1.50 | normal | DisplayR, navigation and footer links |
| Caption | 12.8px (0.80rem) | 400 | 1.10 | normal | DisplayR, small descriptive text |
| Small Label | 12px (0.75rem) | 700 | 1.00 | normal | DisplayR, labels and tags |
| Micro Text | 10px (0.63rem) | 700 | 1.45 | normal | DisplayR, smallest UI text, legal fine print |
| Micro Caption | 8.5px (0.53rem) | 400 | normal | normal | DisplayR, absolute smallest text (legal) |

### Principles
- **Single-family discipline**: DisplayR handles everything from 56px hero headlines to 8.5px legal captions — the font's geometric precision allows it to scale across this extreme range without losing character
- **Bold-default headings**: Weight 700 dominates the heading hierarchy. Unlike brands that use medium (500) for headings, Aurora's Bold weight creates a more assertive, energetic reading experience
...
- **28-degree radical r**: The typeface's signature detail — the lowercase "r" terminal is cut at precisely 28 degrees to mirror the angles of the Aurora diamond logo, embedding brand identity into every word
...
### Buttons
Aurora's buttons are sharp-edged rectangles with zero border-radius — the industrial precision of a pressed metal body panel.

**Super Primary (Yellow)** — The highest-emphasis CTA:
- Default: bg `#EFDF00` (Aurora Yellow), text `#000000`, borderRadius 0px, padding 10px 15px, border 1px solid `#EFDF00`
- Inverted: bg `#EFDF00`, text `#000000` — same yellow on dark backgrounds
- fontSize 16px (DisplayR), fontWeight 700, minHeight 46px, minWidth 46px
...
- fontSize 16px (DisplayR), fontWeight 700
...
- Hover: color shifts to `#1883FD` (Aurora Blue), text-decoration none
...
- Heading: DisplayR 40px/700, white text positioned over gradient
...
- Font: DisplayR, 12.8px
- Focus: standard browser focus ring

### Navigation
- **Desktop**: Aurora diamond logo centered/left, horizontal nav links, sticky positioning
- **Background**: white, no shadow at rest
- **Links**: DisplayR, 13px, weight 700, black text
- **Hover**: color shifts to `#1883FD` (Aurora Blue)
...
### Shadow Philosophy
Aurora uses a richer shadow system than the luxury style or minimal — seven distinct shadow tokens reflecting a more layered, dimensional interface. The shadows progress from subtle 4px hover lifts to dramatic 80px deep panels. The compound shadow (Level 3) with its dual-layer approach (a tight dark shadow plus a wider purple-tinted one from `rgba(50,50,93,0.1)`) is particularly refined — it creates a photorealistic floating effect. The ambient glow (Level 6) in warm gray adds a unique touch that connects to Aurora's warmer color personality.
...
### Do
- Use Aurora Yellow (`#EFDF00`) exclusively for super-primary CTAs — it carries the full weight of the diamond logo's identity
- Maintain zero border-radius on all buttons — sharp edges are non-negotiable in the Aurora system
- Use DisplayR Bold (700) as the default heading weight — the assertive weight is central to the brand's energetic personality
...
- Use `#1883FD` (Aurora Blue) consistently for all link hover states — one interactive color signal
...
### Don't
- Apply Aurora Yellow as a background color for sections or surfaces — it's a CTA signal, not an atmosphere color
- Add border-radius to buttons — the zero-radius rectangle is a core brand marker
- Use any typeface besides DisplayR — the single-family discipline is a brand pillar
- Mix multiple chromatic accent colors in a single section — the palette is monochrome-plus-yellow
- Soften heading weights to 400 or 500 — DisplayR Bold is the brand voice, lighter weights read as off-brand
...
- Navigation links: DisplayR 13px with adequate spacing between items
- Carousel navigation: large arrow targets at viewport edges

### Collapsing Strategy
- **Navigation**: Full horizontal nav collapses to Aurora diamond logo + hamburger menu on mobile
...
- Primary CTA (Super): "Aurora Yellow (#EFDF00)"
...
- Link Hover: "Aurora Blue (#1883FD)"
- Border: "Pale Silver (#F2F2F2)"
- Semantic Error: "Error Rose (#BE6464)"

### Example Component Prompts
- "Create a hero section with a full-viewport aurora gradient background (magenta to violet to teal), a centered vehicle image, a DisplayR Bold headline at 56px with 0.95 line-height in white, and two buttons: a Primary (white bg, black text, 0px radius) 'Explore' and a Ghost (transparent bg, white border, white text, 0px radius) 'Learn More'"
- "Design a PromoCard with a full-bleed photography background, a dark gradient overlay (rgba(0,0,0,0.6) top to transparent at 40%), a DisplayR Bold 40px white heading, a 14px body text line in white, and a Primary inverted button (white bg, black text, 0px radius, 10px 15px padding)"
- "Build a vehicle range grid with 3 columns on white background, each card showing a transparent-background car render above a DisplayR Bold 24px model name in black, a 14px price caption, and a ghost button (black border, black text, 0px radius) labeled 'Configure'"
- "Create a dark E-Tech section on Absolute Black (#000000) with a DisplayR Bold 40px white heading 'E-Tech electric powertrain', a 14px subtitle in white, and a Aurora Yellow (#EFDF00) super-primary button with black text, 0px radius, and 10px 15px padding"
- "Design a search input as a pill-shaped field (50px border-radius) with white background, 1px solid #D1D1D1 border, DisplayR 12.8px text, 6px 35px 6px 15px padding, and a search icon positioned inside the right padding area"

### Iteration Guide
When refining existing screens generated with this design system:
1. Focus on ONE component at a time — Aurora's system has clear component boundaries (PromoCard, VehicleRangeCard, CTA variants)
...
4. Describe the desired "feel" alongside specific measurements — "assertive automotive energy" communicates the DisplayR Bold heading personality better than "font-weight: 700"
5. Always check whether a section should be light or dark — the chessboard alternation is a core pattern
6. Reserve Aurora Yellow for ONE button per screen — if yellow appears in more than one CTA, the hierarchy collapses
