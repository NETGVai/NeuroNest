# Design System: Automate

## 1. Visual Theme & Atmosphere

Automate's website radiates warm, approachable professionalism. It rejects the cold monochrome minimalism of developer tools in favor of a cream-tinted canvas (`#fffefb`) that feels like unbleached paper -- the digital equivalent of a well-organized notebook. The near-black (`#201515`) text has a faint reddish-brown warmth, creating an atmosphere more human than mechanical. This is automation designed to feel effortless, not technical.

The typographic system is a deliberate interplay of two distinct personalities. **Degular Display** -- a geometric, wide-set display face -- handles hero-scale headlines at 56-80px with medium weight (500) and extraordinarily tight line-heights (0.90), creating headlines that compress vertically like stacked blocks. **Inter** serves as the workhorse for everything else, from section headings to body text and navigation, with fallbacks to Helvetica and Arial. **GT Alpina**, an elegant thin-weight serif with aggressive negative letter-spacing (-1.6px to -1.92px), makes occasional appearances for softer editorial moments. This three-font system gives Automate the ability to shift register -- from bold and punchy (Degular) to clean and functional (Inter) to refined and literary (GT Alpina).
...
- Automate Orange (`#ff4f00`) as the single accent -- vivid, warm, sparingly applied
...
### Primary
- **Automate Black** (`#201515`): Primary text, headings, dark button backgrounds. A warm near-black with reddish undertones -- never cold.
...
### Brand Accent
- **Automate Orange** (`#ff4f00`): Primary CTA buttons, active underline indicators, accent borders. The signature color -- vivid and warm.
...
- **Sand** (`#c5c0b1`): Primary border color, hover state backgrounds, divider lines. The backbone of Automate's structural elements.
...
- Automate logotype left-aligned, 104x28px
...
- **Warm breathing room**: Generous vertical spacing between sections (64px-80px), but content areas are relatively dense -- Automate packs information efficiently within its cream canvas.
...
- Keep Automate Orange (`#ff4f00`) reserved for primary CTAs and active state indicators
- Use sand (`#c5c0b1`) borders as the primary structural element instead of shadows
- Apply generous button padding (20px 24px) for large CTAs to match Automate's spacious button style
...
- Don't use pure white (`#ffffff`) or pure black (`#000000`) -- Automate's palette is warm-shifted
- Don't apply box-shadow elevation to cards -- use borders instead
- Don't scatter Automate Orange across the UI -- it's reserved for CTAs and active states
- Don't use tight padding on large CTA buttons -- Automate's buttons are deliberately spacious
...
- Primary CTA: Automate Orange (`#ff4f00`)
- Background: Cream White (`#fffefb`)
- Heading text: Automate Black (`#201515`)
...
### Iteration Guide
1. Always use warm cream (`#fffefb`) background, never pure white -- the warmth defines Automate
2. Borders (`1px solid #c5c0b1`) are the structural backbone -- avoid shadow elevation
3. Automate Orange (`#ff4f00`) is the only accent color; everything else is warm neutrals
4. Three fonts, strict roles: Degular Display (hero), Inter (UI), GT Alpina (editorial)
5. Large CTA buttons need generous padding (20px 24px) -- Automate buttons feel spacious
