...
- **Leaf Green** (`#00ed64`): Primary brand accent — neon green for highlights, underlines, gradients
...
### Font Families
- **Display Serif**: `Leaf Value Serif` — editorial hero headlines
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Leaf Value Serif | 96px (6.00rem) | 400 | 1.20 (tight) | normal | Serif authority |
| Display Secondary | Leaf Value Serif | 64px (4.00rem) | 400 | 1.00 (tight) | normal | Serif sub-hero |
...
### Principles
- **Serif for authority**: Leaf Value Serif at hero scale creates an editorial presence unusual in tech — it communicates that Leaf is an institution, not a startup.
...
- Background: `#00684a` (muted Leaf green)
- Text: `#000000`
...
- Leaf logo (leaf icon + wordmark) left-aligned
...
**Shadow Philosophy**: Leaf's shadow system is unique in that the primary elevation shadow uses `rgba(0, 30, 43, 0.12)` — a teal-tinted shadow that carries the forest-dark brand color into the depth system. This means even on white surfaces, shadows feel like they belong to the Leaf color world rather than being generic neutral black.

## 7. Do's and Don'ts

### Do
- Use `#001e2b` (forest-black) for dark sections — not pure black
- Apply Leaf Green (`#00ed64`) sparingly for maximum electric impact
- Use Leaf Value Serif ONLY for hero/display headings — Euclid Circular A for everything else
...
- Don't use Leaf Green (`#00ed64`) on backgrounds — it's an accent for text, underlines, and small highlights
- Don't use standard gray shadows — always use teal-tinted (`rgba(0,30,43,...)`)
- Don't apply serif font to body text — Leaf Value Serif is hero-only
...
### Collapsing Strategy
- Hero: Leaf Value Serif 96px → 64px → scales further
...
- Brand accent: Leaf Green (`#00ed64`)
...
- "Create a hero on forest-black (#001e2b) background. Headline at 96px Leaf Value Serif weight 400, line-height 1.20, white text with 'potential' highlighted in Leaf Green (#00ed64). Subtitle at 18px Euclid Circular A weight 400. Green pill CTA (#00684a, 100px radius). Neon green gradient glow behind product screenshot."
...
- "Build a dark section: #001e2b background, 1px solid #3d4f58 border on cards. White text. Leaf Green (#00ed64) accent underlines on headings using bottom-border 2px solid."
...
2. Leaf Green (#00ed64) is electric — use once per section for maximum impact
3. Serif headlines (Leaf Value Serif) create the editorial authority — never use for body
...
6. Teal-tinted shadows keep everything in the Leaf color world
